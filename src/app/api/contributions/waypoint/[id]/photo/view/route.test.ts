...
const mocks = vi.hoisted(() => ({
  ...
  rows: [] as Array<{ photoUrl: string | null }>,
}));
...
const req = () =>
  new Request("http://localhost/api/contributions/waypoint/w1/photo/view");
...
beforeEach(() => {
  ...
  mocks.rows = [{ photoUrl: `${BLOB}/contributions/waypoints/u1/x.jpg` }];
  process.env.BLOB_READ_WRITE_TOKEN = "vercel_blob_rw_dummy";
});

describe("GET /api/contributions/waypoint/[id]/photo/view", () => {
  ...
  it("returns 404 when the suggestion has no photo", async () => {
    mocks.rows = [{ photoUrl: null }];
    const res = await GET(req(), ctx("w1"));
    expect(res.status).toBe(404);
    expect(mocks.get).not.toHaveBeenCalled();
  });
